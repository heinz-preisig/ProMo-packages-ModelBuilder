...
class Initialization(object):
	...
	def recursiv(self):
		...
		for state in self.states:
			...
			val = self.locateAndCheckVar(state, self.variableTree[state])
			if val == True:
	...
	def locateAndCheckVar(self, variable, tree):
		statement = False
		keys, topkey, varTree = self.rec_locateState(tree, variable, variable)
		if keys == []:
			return statement
		else:
			keys.append(topkey)
			...
			for var in keys:
				...
				if var in self.states:
					...
					counter += 1
					if var in self.compVarSet_final:
						pass
					else:
						self.compVarSet_final.append(var)
					...
					if var == topkey:
						if topkey == self.runningState:
							...
							self.layer += 1
							val = self.locateAndCheckVar(topkey, self.variableTree)
							self.layer -= 1
							if val == True:
								counter += 1
					else:
						...
						if val == True:
							...
							self.layer += 1
							val = self.locateAndCheckVar(var, self.variableTree)
							self.layer -= 1
							if val == True:
								counter += 1
		...
		if statement == False:
			...
			for keys in varTree:
				if not varTree[keys]:
					...
					val = self.locateAndCheckVar(variable, varTree[keys])
					if val == True:
						statement = True
						break
		return statement
	...
	def recFuncVarCheck(self,treeDict):
		...
		for key in treeDict:
			...
			if key in self.compVarSet:
				counter += 1
				if key in self.compVarSet_final:
						pass
				else:
					self.compVarSet_final.append(key)
	...
	def eq_builder(self, variable):
		...
		self.vars_used_VT.append(variable)
		if len(eq_needed) > 1:
			for eq in eq_needed:
				if eq in self.eq_order:
...
if __name__ == '__main__':
	...
	test = Initialization('cc_HEX_single_nodes.json', 'equations.json', 'variables.json', 'typed_tokens.json', ['n','H'])
	test.equationSetBuilder()
	print(test.eq_used)
	print(test.eq_order)
