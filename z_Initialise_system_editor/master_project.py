...
class Initialization(object):
	...
	def __init__(self, filename_model, filename_equations, filename_variables, filename_typed_tokens, states):
		...
		self.eq_implicit = [] # list containing variables with the potential of being implict in the current part of the
							  # variable tree that is being investigated
		...
		if children  == []:
			...
			for child in children:
				if self.data['nodes'][str(child)]["type"] == 'constant':
					index = children.index(child)
					temp_index.append(copy.copy(index))
			for ind in list(reversed(temp_index)):
				children.pop(ind)
			self.groupDict[node] = {}
	...
	def separateArcs(self):
		...
		for groups, items in self.groupArcs.items():
			nexts = False
			for group, item in self.groupDict.items():
				if groups in item['groupChildren']:
					nexts = True
			if nexts:
				continue
			...
			for group, item in self.groupArcs.items():
				nexts2 = False
				for group2, item2 in self.groupDict.items():
					if group in item2['groupChildren']:
						nexts2 = True
				if nexts2:
					continue
	...
	def recFunc(self,eq_needed):
		if self.eq_used.__contains__(eq_needed):
			index = self.eq_used.index(eq_needed)
			self.eq_used.pop(index)
		...
		for var in vars_needed:
			...
			if self.variables[var]['equation_list'] == []:
				...
				if self.variables[var]["type"] == 'state':
					self.vars_used_const.append(var)
					continue
				elif var in self.eq_implicit:
					if var in self.vars_used_const:
						pass
					else:
						self.vars_used_const.append(var)
					continue
				else:
					self.eq_implicit.append(var)
				...
				index = self.eq_implicit.index(var)
				self.eq_implicit.pop(index)
				...
				for nodes, items in self.reservoirDict.items():
					if self.variables[var]['index_structures'] == ['node & species']:
						if items['orderOfspecies'] == None:
							items[key][var] = np.ones((1,1)).tolist()
						else:
							items[key][var] = np.ones((len(items['orderOfspecies']),1)).tolist()
					else:
						items[key][var] = np.ones((1,1)).tolist()
					...
					if self.variables[var]['index_structures'] == ['node & species']:
						...
						for i in range(len(items['orderOfspecies'])):
							if items['orderOfspecies'][i] == None:
								temp = np.ones((1,1)).tolist()
							else:
								temp = np.ones((len(items['orderOfspecies'][i]),1)).tolist()
							array.append(temp)
						items[key][var] = array
					else:
						array = []
						for i in range(len(items['orderOfspecies'])):
							array.append(np.ones((1,1)).tolist())
						items[key][var] = array
	...
	def addConstNotNodeOrArc(self, var):
		...
		for token, item in self.typed.items():
			...
			if indexing == token:
				...
				self.constNotNodeOrArc[var]['value'] = np.ones((len(item['instances']),1)).tolist()
				...
				self.constNotNodeOrArc[var] = {}
				self.constNotNodeOrArc[var]['value'] = np.ones((len(conversions),1)).tolist()
				...
				for node, item in self.singleDict:
					...
					if item['conversions'] == None:
						...
						order[-1] = slot
				self.constNotNodeOrArc[var] = {}
				self.constNotNodeOrArc[var]['value'] = np.ones((size,1)).tolist()
				self.constNotNodeOrArc[var]['order'] = order
				self.constNotNodeOrArc[var]['nodeOrder'] = nodesOrder
				temp = False
		if temp:
			self.constNotNodeOrArc[var] = {}
			self.constNotNodeOrArc[var]['value'] = np.ones((1,1)).tolist()
		...
		for eqs in self.eq_used:
			...
			if state_eq_indexes.__contains__(index) or eqs == self.eq_used[-1]:
				if eqs == self.eq_used[-1]:
					temp.append(eqs)
				self.temp_order.insert(0,list(reversed(temp)))
				temp = []
				temp.append(eqs)
			else:
				temp.append(eqs)
	...
	def addVariablesAndStates(self):
		...
		for arcs, items in self.groupArcsBetweenNode.items():
			for tokens, item in items['tokens'].items():
				item['vars'] = {}
		self.addVarSet(self.compVarSet_final)
	...
	def recursiv(self):
		...
		self.tempImp = []
		for state in self.states:
			if state in self.calculatedStates:
				continue
			self.runningState = state
			val = self.locateAndCheckVar(state, state, self.variableTree[state])
			if val == True:
				self.calculatedStates.append(state)
				self.recursiv()

	def calcStates(self):
		self.calculatedStates = []
		for state in self.states:
			if state in self.compVarSet:
				self.calculatedStates.append(state)
				self.compVarSet_final.append(state)

	def locateAndCheckVar(self, top, variable, tree):
		statement = False
		keys, topkey, varTree = self.rec_locateState(tree, top, variable)
		if keys == []:
			return statement
		elif self.layer > 5:
			return statement
		elif topkey == self.runningState:
			return statement
		else:
			keys.append(topkey)
			counter = 0
			...
			for var in keys:
				...
				if var in self.states:
					if var == variable:
						counter +=1
					elif var in self.calculatedStates:
						counter += 1
					else:
						continue
				elif var in self.compVarSet:
					self.tempVars.append(var)
					counter += 1
				elif var in self.vars_used_VT:
					counter += 1
					...
					if var == topkey:
						if topkey == self.runningState:
							continue
						else:
							self.layer += 1
							val = self.locateAndCheckVar(self.runningState, topkey, self.variableTree[self.runningState])
							self.layer -= 1
							if val == True:
								counter += 1
					else:
						val = self.recFuncVarCheck(varTree[var])
						if val == True:
						 	eq_set = self.eq_builder(var)
						 	self.tempSet.append(eq_set)
						 	counter += 1
						else:
							self.layer += 1
							val = self.locateAndCheckVar(self.runningState, var, self.variableTree[self.runningState])
							self.layer -= 1
							if val == True:
								counter += 1

			if counter == check:
				eq_needed = self.variables[topkey]['equation_list']
				if len(eq_needed) > 1:
					for eq in eq_needed:
						if eq in self.eq_order:
							self.tempImp.append(eq)
				else:
					self.tempImp.append(eq_needed[0])
				if self.layer == 1:
					for eq in self.tempSet:
						self.eq_order_VT.append(eq)
					self.tempSet = []
					for tempvar in self.tempVars:
						self.vars_used_VT.append(tempvar)
					self.tempVars = []
					for eq in self.tempImp:
						self.eq_order_VT.append(eq)
						self.eq_used_VT.append(eq)

					for newvar in self.vars_used_VT:
						if newvar in self.compVarSet:
							if newvar in self.compVarSet_final:
								pass
							else:
								self.compVarSet_final.append(newvar)
				statement = True

		if statement == False:
			for eqs in self.tempSet:
				if eqs == []:
					continue
				for eq in eqs:
					if eq in self.eq_used_VT:
						index = self.eq_used_VT.index(eq)
						self.eq_used_VT.pop(index)
			self.tempSet = []
			self.tempImp = []
			self.tempVars = []

			for keys in tree:
				if keys in self.states:
					continue
				if not tree[keys]:
					continue
				else:
					val = self.locateAndCheckVar(keys, variable, tree[keys])
					if val == True:
						statement = True
						break
		return statement

	def rec_locateState(self, varTree, top, seekVar):
		self.found = False
		if not varTree:
			keys = []
			top_new = ''
			varTree_new = {}
			return keys, top_new, varTree_new
		else:
			if seekVar in list(varTree.keys()):
				'''
				self.keys = list(varTree.keys())
				self.topkey = top
				self.varTree =  varTree
				'''
				keys  = list(varTree.keys())
				top_new = top
				varTree_new = varTree
				self.found = True
				return keys, top_new, varTree_new
				...
				for key in varTree:
					keys, top_new, varTree_new = self.rec_locateState(varTree[key], key, seekVar)
					if self.found:
						return keys, top_new, varTree_new
				return keys, top_new, varTree_new

	def recFuncVarCheck(self,treeDict):
		complete = False
		counter = 0
		check = 0
		for key in treeDict:
			...
			if key in self.compVarSet:
				counter += 1
			elif key in self.calculatedStates:
				counter += 1
			else:
				branch = treeDict[key]
				if not branch:
					if self.variables[key]["type"] == 'constant':
						val = True
					else:
						val = False
				else:
					val = self.recFuncVarCheck(branch)
				if val == True:
					counter += 1
		if check == counter:
			complete = True
	...
	def eq_builder(self, variable):
		self.eq_set = []
		eq_needed = self.variables[variable]['equation_list']
		self.tempVars.append(variable)
		if len(eq_needed) > 1:
			for eq in eq_needed:
				if eq in self.eq_order:
					self.eq_set.append(eq)
					self.recFuncEqVT(eq)
		elif len(eq_needed) == 1:
			self.eq_set.append(eq_needed[0])
			self.recFuncEqVT(eq_needed[0])
		return list(reversed(self.eq_set))

	def recFuncEqVT(self,eq_needed):
		if self.eq_used_VT.__contains__(eq_needed):
			...
			vars_needed = self.equations[eq_needed]['incidence_list']
			for var in vars_needed:
				if self.compVarSet.__contains__(var):
					continue
				elif self.variables[var]['equation_list'] == []:
					continue
				elif var in self.vars_used_VT:
					...
					if len(equations_needed) > 1:
						...
						self.eq_set.append(equations_needed[index])
						self.recFuncEqVT(equations_needed[index])
					else:
						self.eq_set.append(equations_needed[0])
						self.recFuncEqVT(equations_needed[0])

	### Main function ###

	def calculateInitState(self, varSet = []):
		self.makeVariableTree()
		self.compVarSet = varSet
		self.calcStates()
		self.recursiv()
		if Counter(self.calculatedStates) == Counter(self.states):
			message = 'The initial states can be calculated using the following set {}'.format(self.compVarSet_final)
		else:
			message = 'Variable set is not complete'
			self.compVarSet_final = []
			self.eq_order_VT = []
			self.calculatedStates = []
			self.vars_used_VT = []
	...
	def updatedDict(self, update, key = '', piece = ''):
				...
				if node == key:
					for child in item['children']:
						...
						for var in item['states']:
							self.outputNodes[str(child)][var] = str(item['states'][var][index].tolist())
						for var in item['vars']:
							self.outputNodes[str(child)][var] = str(item['vars'][var][index].tolist())
			message = 'Node {} has been intialised'.format(key)
					...
					if arc == key:
						for token, items in item['tokens'].items():
							for arcs in items['arcs']:
								...
								for var in items['vars']:
									self.outputArcs[arcs][var] = str(items['vars'][var][index].tolist())
					...
					if arc == key:
						for token, items in item['tokens'].items():
							for arcs in items['arcs']:
								...
								for var in items['vars']:
									self.outputArcs[arcs][var] = str(items['vars'][var][index].tolist())
			message = 'Arc {} has been intialised'.format(key)
		else:
			if piece == 'node':
				if key in self.initializedNodes:
					pass
				...
				message = 'Constants has been intialised'
		return message

	def loadNode(self, loadDict):
		for key in loadDict:
			if key in self.singleDict:
				item = self.singleDict[key]
				for var in loadDict[key]:
					if self.variables[var]["type"] == 'state':
						typ = 'state'
					else:
						typ = 'vars'
						item[typ][var] = eval(loadDict[key][var])
			elif key in self.reservoirDict:
				item = self.reservoirDict[key]
				for var in loadDict[key]:
					if self.variables[var]["type"] == 'state':
						typ = 'state'
					else:
				...
				for groups, item in self.groupDict.items():
					if key in item['children']:
						index = item['children'].index(key)
						for var in loadDict[key]:
							if self.variables[var]["type"] == 'state':
								typ = 'state'
							else:
								typ = 'vars'
							item[typ][var][index] = eval(loadDict[key][var])

	def loadArc(self, loadDict):
		for key in loadDict:
			if key in self.singleArcs:
				self.singleArcs[key]['vars'] = eval(loadDict[key])
			else:
				for group in self.groupArcs:
					feature = self.groupArcs
					for token, item in feature['tokens'].items():
						if key in item['arcs']:
							index = item['arcs'].index(key)
							item['vars'][var][index] = eval(loadDict[key][var])
				for group in self.groupArcsBetweenNode:
					feature = self.groupArcsBetweenNode
					for token, item in feature['tokens'].items():
						if key in item['arcs']:
							index = item['arcs'].index(key)
							item['vars'][var][index] = eval(loadDict[key][var])

	def loadConst(self, loadDict):
		for var in loadDict:
			self.constNotNodeOrArc[var]['value'] = eval(loadDict[var])
	...
	def assembleOutput(self):
		...
		for const in self.constNotNodeOrArc:
			self.outputConst[const] = copy.copy(self.constNotNodeOrArc[const]['value'])
...
if __name__ == '__main__':
	...
	test = Initialization('cc_HEX_all_groups.json', 'equations.json', 'variables.json', 'typed_tokens.json', ['n'])
	test.equationSetBuilder()
	print(test.eq_order)
	#print(test.calculateInitState(['T','p']))
	#print(test.eq_order_VT)
